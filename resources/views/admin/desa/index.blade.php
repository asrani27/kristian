        ...
        <div class="p-6 border-b border-gray-100">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Kelola data desa</p>
                </div>
                <div class="flex flex-col sm:flex-row gap-3">
                    <form action="{{ route('admin.desa.index') }}" method="GET" class="relative">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    @if($desa->user)
                                        <form action="{{ route('admin.desa.resetPassword', $desa->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" 
                                                onclick="return confirm('Apakah Anda yakin ingin reset password akun desa ini?')"
                                                class="p-2 rounded-lg bg-green-50 text-green-600 hover:bg-green-100 transition" title="Reset Password">
                                                <i class="fas fa-key"></i>
                                            </button>
                                        </form>
                                    @else
                                        <form action="{{ route('admin.desa.createAkun', $desa->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" 
                                                onclick="return confirm('Apakah Anda yakin ingin membuat akun untuk desa ini?')"
