...
@section('breadcrumb', 'Edit Data Kepala Desa')
    ...
    <div class="max-w-2xl">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h3 class="text-lg font-bold text-gray-800">Edit Kepala Desa</h3>
                    <p class="text-sm text-gray-500">Edit data kepala desa</p>
                </div>
            </div>
                ...
                <div class="space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" name="nik" id="nik" value="{{ old('nik', $kepalaDesa->nik) }}" required
                        ...
                        <div>
                            ...
                            <input type="text" name="nama" id="nama" value="{{ old('nama', $kepalaDesa->nama) }}" required
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <select name="desa_id" id="desa_id" required
                                ...
                                @foreach($desas as $desa)
                                    <option value="{{ $desa->id }}" {{ old('desa_id', $kepalaDesa->desa_id) == $desa->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="status" id="status" required
                                ...
                                <option value="aktif" {{ old('status', $kepalaDesa->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                                <option value="demission" {{ old('status', $kepalaDesa->status) == 'demission' ? 'selected' : '' }}>Demisioner</option>
                                <option value="nonaktif" {{ old('status', $kepalaDesa->status) == 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="date" name="tanggal_menjabat" id="tanggal_menjabat" value="{{ old('tanggal_menjabat', $kepalaDesa->tanggal_menjabat ? \Carbon\Carbon::parse($kepalaDesa->tanggal_menjabat)->format('Y-m-d') : '') }}" required
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_demisioner" id="tanggal_demisioner" value="{{ old('tanggal_demisioner', $kepalaDesa->tanggal_demisioner ? \Carbon\Carbon::parse($kepalaDesa->tanggal_demisioner)->format('Y-m-d') : '') }}"
                    ...
                    <div>
                        ...
                        <textarea name="alamat" id="alamat" rows="3"
                            ...
                            placeholder="Masukkan alamat">{{ old('alamat', $kepalaDesa->alamat) }}</textarea>
                        @error('alamat')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @if($kepalaDesa->foto)
                            <div class="mb-3">
                                <img src="{{ asset('storage/' . $kepalaDesa->foto) }}" alt="{{ $kepalaDesa->nama }}" class="w-24 h-24 rounded-xl object-cover">
                            </div>
                        @endif
