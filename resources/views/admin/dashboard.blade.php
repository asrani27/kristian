...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6 lg:mb-8">
    ...
    <div class="stat-card bg-white rounded-2xl shadow-sm border border-gray-100 p-4 lg:p-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <p class="text-xs lg:text-sm text-gray-500 font-medium">Total Desa</p>
                <p class="text-2xl lg:text-3xl font-bold text-gray-800">{{ $stats['total_desa'] }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="stat-card bg-white rounded-2xl shadow-sm border border-gray-100 p-4 lg:p-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <p class="text-xs lg:text-sm text-gray-500 font-medium">Total Kecamatan</p>
                <p class="text-2xl lg:text-3xl font-bold text-gray-800">{{ $stats['total_kecamatan'] }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="stat-card bg-white rounded-2xl shadow-sm border border-gray-100 p-4 lg:p-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <p class="text-xs lg:text-sm text-gray-500 font-medium">Kepala Desa</p>
                <p class="text-2xl lg:text-3xl font-bold text-gray-800">{{ $stats['total_kepala_desa'] }}</p>
            </div>
        </div>
    </div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
    ...
    <div class="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-4 lg:px-6 py-4 lg:py-5 border-b border-gray-100 flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5">Kegiatan terbaru dari sistem</p>
            </div>
            <a href="#" class="text-primary hover:text-sky-600 text-sm font-medium flex items-center gap-1 transition">
                Lihat Semua
        ...
        <div class="p-4 lg:p-6">
                ...
                <div class="space-y-4">
                    @foreach($recentActivities as $activity)
                        <div class="flex items-start gap-4 p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition">
                            <div class="w-10 h-10 lg:w-12 lg:h-12 bg-primary/10 rounded-xl flex items-center justify-center flex-shrink-0">
                                <i class="fas fa-calendar-check text-primary"></i>
                            </div>
                            <div class="flex-1 min-w-0">
                                <h4 class="font-medium text-gray-800 truncate">{{ $activity->nama_kegiatan }}</h4>
                                <p class="text-sm text-gray-500 mt-1">
                                    <i class="fas fa-map-marker-alt mr-1"></i>
                                    {{ $activity->desa->nama_desa ?? 'N/A' }}
                                </p>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <a href="#" class="inline-block mt-4 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-sky-600 transition">
                        Tambah Kegiatan
                    </a>
                </div>
            @endif
    ...
    <div class="space-y-4 lg:space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 lg:p-6">
            ...
            <div class="space-y-3">
                <a href="#" class="flex items-center gap-3 p-3 rounded-xl bg-primary/5 hover:bg-primary/10 text-primary transition">
                    <div class="w-10 h-10 bg-primary rounded-lg flex items-center justify-center">
                        <i class="fas fa-plus text-white text-sm"></i>
                    </div>
                    <span class="font-medium">Tambah Desa Baru</span>
                </a>
                <a href="#" class="flex items-center gap-3 p-3 rounded-xl bg-purple-500/5 hover:bg-purple-500/10 text-purple-600 transition">
                    <div class="w-10 h-10 bg-purple-500 rounded-lg flex items-center justify-center">
                        <i class="fas fa-user-plus text-white text-sm"></i>
                    </div>
                    <span class="font-medium">Tambah Kepala Desa</span>
                </a>
                <a href="#" class="flex items-center gap-3 p-3 rounded-xl bg-green-500/5 hover:bg-green-500/10 text-green-600 transition">
                    <div class="w-10 h-10 bg-green-500 rounded-lg flex items-center justify-center">
                        <i class="fas fa-clipboard text-white text-sm"></i>
                    </div>
                    <span class="font-medium">Buat Kegiatan</span>
                </a>
            </div>
        </div>

        <!-- System Info -->
        <div class="bg-gradient-to-br from-primary to-secondary rounded-2xl shadow-lg p-4 lg:p-6 text-white">
            <div class="flex items-center gap-3 mb-4">
                <div class="w-10 h-10 bg-white/20 rounded-lg flex items-center justify-center">
                    <i class="fas fa-info-circle"></i>
                </div>
                <div>
                    <h3 class="font-bold">Informasi Sistem</h3>
                    <p class="text-xs text-white/80">Status & Statistik</p>
                </div>
            </div>
            <div class="space-y-3">
                <div class="flex items-center justify-between">
                    <span class="text-sm text-white/80">Status</span>
                    <span class="px-2 py-1 bg-white/20 rounded-full text-xs font-medium flex items-center gap-1">
                        <span class="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span>
                        Online
                    </span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-sm text-white/80">Versi</span>
                    <span class="text-sm font-medium">1.0.0</span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-sm text-white/80">Total Data</span>
                    <span class="text-sm font-medium">{{ $stats['total_desa'] + $stats['total_kecamatan'] + $stats['total_kepala_desa'] }} Records</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 lg:p-6">
            <div class="text-center">
                ...
                <p class="text-sm text-gray-500 capitalize">{{ Auth::user()->role === 'admin' ? 'Administrator' : Auth::user()->role }}</p>
